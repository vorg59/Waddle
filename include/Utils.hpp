...
namespace utils
{
	...
	inline void PrintScoresP1(vector<Entity>& Numbers,RenderWindow& window, Movement& M, int Window_Width, int Window_Height)
	{
        int next=-8;
        for (int j = 0; j <= M.getP1_Score(); ++j)
        {
            Entity e = utils::getScore(Numbers, j);
            e.setPos(Vector2f(next,15*0.2));
            window.render(e, 0.2);
            next+=12;
        }

    }
    inline void PrintScoresP2(vector<Entity>& NumbersCrew,RenderWindow& window, Movement& M, int Window_Width, int Window_Height)
    {
        int next = 1280;
        next*=0.2;
        for (int j = 0; j <= M.getP2_Score(); ++j)
        {
            Entity e = utils::getScore(NumbersCrew, j);
            e.setPos(Vector2f(next,15*0.2));
            window.render(e, 0.2);
            next-=12;
        }
